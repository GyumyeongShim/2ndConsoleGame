...
bool VisualEffectEvent::Update(Wannabe::BattleContext& context, float fDeltaTime)
{
    ...
    clearCell.wch = L' ';
    renderSys.GetEffectCanvas().SetCell(m_LastPos.x, m_LastPos.y, clearCell);
    ...
    {
        // 선형 보간 (Lerp)
        m_CurrentPos.x = static_cast<int>(m_StartPos.x + (m_EndPos.x - m_StartPos.x) * t);
        m_CurrentPos.y = static_cast<int>(m_StartPos.y + (m_EndPos.y - m_StartPos.y) * t);
    }
    else if (m_eMoveType == EffectMovementType::Parabola)
    {
        ...
        m_CurrentPos.y = static_cast<int>((m_StartPos.y + (m_EndPos.y - m_StartPos.y) * t) - jump);
    }
    ...
    newCell.bDirty = true;
    renderSys.GetEffectCanvas().SetCell(m_CurrentPos.x, m_CurrentPos.y, newCell);
...
void VisualEffectEvent::OnEnd(Wannabe::BattleContext& context)
{
    CanvasCell cell;
    cell.wch = L' ';
    Engine::Get().GetRenderSystem().GetEffectCanvas().SetCell(m_CurrentPos.x, m_CurrentPos.y,cell);
}
